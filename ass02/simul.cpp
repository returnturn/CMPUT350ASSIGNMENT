...
int main(int argc, char *argv[])
{
  ...
  for (int i=0; i < n_marines; ++i) {
    ...
    // mirrored    
    Unit *v = new Marine(BLUE, world->rnd_pos(mr), blue_marine_policy, bounce);
    v->heading = world->rnd_heading(); 
  ...
  for (int i=0; i < n_tanks; ++i) {
    ...
    // mirrored    
    Unit *v = new Tank(BLUE, world->rnd_pos(tr), blue_tank_policy, bounce);
    v->heading = world->rnd_heading(); 
    v->current_speed = v->max_speed;    
    world->units.push_back(v);
  }
  ...
  if (s >= MAX_STEPS) {
    cout << "game over (timeout seed=" << seed << "): draw" << endl;
    ++draws;
  }
...
int main(int argc, char *argv[]){
  int idc;
  for(int i = 0; i<100; ++i){
    idc = oldmain(argc, argv);
  }
  cout << idc << endl;
  cout << "RED: " << redwins << endl;
  cout << "BLUE: " << bluewins << endl;
